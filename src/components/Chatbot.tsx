...
import { sendMessageToGemini, isGeminiConfigured } from "../services/chatbotService";
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (isGeminiConfigured()) {
        try {
          botResponse = await sendMessageToGemini(input);
        } catch (geminiError) {
          console.error("Gemini failed, trying OpenAI...", geminiError);
          if (isOpenAIConfigured()) {
            botResponse = await sendMessageToOpenAI(input);
          } else {
            throw new Error("Both Gemini and OpenAI APIs are not configured");
          }
        }
      } else if (isOpenAIConfigured()) {
        botResponse = await sendMessageToOpenAI(input);
      } else {
        throw new Error("No AI API keys configured. Please add VITE_GEMINI_API_KEY or VITE_OPENAI_API_KEY to your .env file");
      }
    ...
    <div className="fixed bottom-6 right-6 z-50">
      ...
      {isOpen ? (
        ...
        <button
          ...
          className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-full p-4 shadow-lg hover:shadow-xl transition-all duration-300 flex items-center justify-center"
        >
