...
const App: React.FC = () => {
  // Render Chatbot only for authenticated users with a set role and not on auth/public routes
  const AuthChatbot: React.FC = () => {
    const { user, role } = useAuth();
    const location = useLocation();

    // Hide on landing, auth, and fill-details routes
    const hidePaths = ['/', '/auth'];
    if (!user || !role) return null;
    if (hidePaths.includes(location.pathname) || location.pathname.startsWith('/FillDetails')) return null;
    return <Chatbot />;
  };
  return (
    <Router>
